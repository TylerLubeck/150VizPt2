...
public class ColorScheme {
    ...
    public void minimize_LAB() {
      //if (true) return;
      Random rng = new Random();
      float[] best_AvgSd = new float[2]; /* Current best standard deviation */
      float[] curr_AvgSd;
      best_AvgSd[0] = 0.0; best_AvgSd[1] = 10000.0;
      float avg1,avg2,sd1,sd2;
      Color[] best_colors = new Color[colors.length]; /* current best colors */
      
      for (int i = 0; i < best_colors.length; i++) {
        best_colors[i] = new Color("CIELAB", 0.0, 0.0, 0.0);
      }

      for (int i = 0; i < 100000; i++) {
        space = "RGB";
        for (int j = 0; j < colors.length; j++) {
          colors[j] = new Color("RGB", rng.nextInt(256), rng.nextInt(256), rng.nextInt(256));
        }
        toSpace("CIELAB");
        
        curr_AvgSd = getAvgSd();
        avg1 = curr_AvgSd[0]; sd1 = curr_AvgSd[1];
        avg2 = best_AvgSd[0]; sd2 = best_AvgSd[1];
        //if ((sd1 / 1.0) - (avg1 / 50.0) < (sd2 / 1.0) - (avg2 / 50.0)) {
        //if (avg1 > avg2) {
        if (sd1 < sd2) {
          //if (avg1 > avg2) {
            best_AvgSd = getAvgSd();
            for (int j = 0; j < colors.length; j++) {
              best_colors[j] = colors[j];
            }
          //}
        }
      }
      for (int i = 0; i < colors.length; i++) {
        colors[i] = best_colors[i];
      }
      toSpace("RGB");
...
/*
      for (int i = 0; i < 100000; i++) {
        space = "RGB";
        for (int j = 0; j < colors.length; j++) {
          colors[j].channels[0].value = rng.nextInt(256);
          colors[j].channels[1].value = rng.nextInt(256);
          colors[j].channels[2].value = rng.nextInt(256);
        }
        toSpace("CIELAB");
        computeDistance();
        curr_AvgSd = getAvgSd();
        avg1 = curr_AvgSd[0]; sd1 = curr_AvgSd[1];
        avg2 = best_AvgSd[0]; sd2 = best_AvgSd[1];
        if ((avg1 / 50.0) - (sd1 / 5.0) < (avg2 / 50.0) - (sd2 / 5.0)) {
          toSpace("RGB");
          best_AvgSd[0] = avg1;
          best_AvgSd[1] = sd1;
          for (int k = 0; k < best_colors.length; k++) {
            best_colors[k].channels[0].value = colors[k].channels[0].value;
            best_colors[k].channels[1].value = colors[k].channels[1].value;
            best_colors[k].channels[2].value = colors[k].channels[2].value;
          }
        }
      }
      ...
      for (int k = 0; k < best_colors.length; k++) {
        colors[k].space = "RGB";
        colors[k].channels[0].value = best_colors[k].channels[0].value;
        colors[k].channels[1].value = best_colors[k].channels[1].value;
        colors[k].channels[2].value = best_colors[k].channels[2].value;
        colors[k].channels[0].name = "L";
        colors[k].channels[1].name = "a";
        colors[k].channels[2].name = "b";
      }
      computeDistance();
*/
